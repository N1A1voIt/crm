...
public interface ExpensesRepository extends JpaRepository<Expens,Integer> {
    ...
    @Query("SELECT e FROM Expens e JOIN FETCH e.tickets WHERE e.tickets IS NOT NULL")
    List<Expens> findByTicketNotNull();
    void deleteByLead(Integer lead);
    void deleteByTicket(Integer ticket);
    List<Expens> findByLeadNotNull();
}
